...
import TabsImageList from './components/TabsImageList'
...
class App extends Component {
  state = {
    ...
    imagesId: imagesList[0].category,
    count: 0,
    isWriteAnswer: true,
    timer: 60,
  }
  ...
  selectImagesId = category => {
    const {imagesId} = this.state
    if (imagesId === category) {
      this.setState(prevState => ({count: prevState.count + 1}))
    } else {
      this.setState({isWriteAnswer: false})
    }
  }

  getRandomImage = () => {
    const randomPic = imagesList[Math.floor(Math.random() * imagesList.length)]
    this.setState({
      randomImage: randomPic.imageUrl,
      imagesId: randomPic.category,
    })
  }
  ...
  render() {
    ...
    const {randomImage, count, isWriteAnswer, timer} = this.state
    return (
      <div className="container">
        <div>
          <ul>
            <Navbar counter={count} time={timer} />
          </ul>
          ...
          {isWriteAnswer ? (
            <div className="appContainer">
              <ul>
                <li>
                  <img src={randomImage} alt="match" className="match" />
                </li>
              </ul>
            ...
            <Result
              counter={count}
              result={isWriteAnswer}
              reStarter={this.reStartGame}
            />
          )}
        </div>
      </div>
